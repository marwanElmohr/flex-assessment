...
import { fetchApproved, fetchReviews } from "../api/ReviewAPI";
...
export default function PropertyReviews() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const uniqueListings = Array.from(
        new Set(all.map((r) => r.listingName).filter(Boolean))
      );
      setListings(uniqueListings);
      const first = uniqueListings[0] || "";
      setListing(first);

      const approvedReviews = await fetchApproved({ listingName: first });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setReviews(data.filter((r) => r.privateFeedback === 1));
    })();
  }, [listing]);
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      <section className="hero mb-6">
        ...
        <p className="text-gray-600">Only approved reviews are shown here.</p>
      </section>
